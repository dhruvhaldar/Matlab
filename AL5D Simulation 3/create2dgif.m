...
thvec2 = svec(dr*(rvec - rlims(1)) > 0 & dr*(rvec - rlims(2)) < 0);
thvec2 = [slims(1) thvec2 slims(2)];
pause(0.02);
hFig = figure(1);
set(hFig, 'Position', [100 100 1000 700])
h = plot(rvec,thvec,'k',rvec,svec,'b',rvec,evec,'r',rf(n),tf(n),'b*',rf(n),ef(n),'r*',[rvec(n) rvec(n)],[0 200],'c:',[rvec(1) rvec(end)],[sp sp],'b:',[rvec(1) rvec(end)],[sm sm],'b:',[rvec(1) rvec(end)],[sp2 sp2],'b:',[rvec(1) rvec(end)],[sm2 sm2],'b:',[rvec(1) rvec(end)],[ep ep],'r:',[rvec(1) rvec(end)],[em em],'r:',[rp rp],[0 200],'m:',[rp2 rp2],[0 200],'m:',[rm rm],[0 200],'m:',[rm2 rm2],[0 200],'m:',rvec2,thvec2,'g');
